...
uint64_t Power_Of(const uint8_t base, const uint8_t power);
uint16_t Get_Validate_Input_Number(void *const int32_input_num_ptr, char *const input_str_ptr, const unsigned int input_str_max_chars, const int64_t valid_min_value, const int64_t valid_max_value);
...
int main()
{
	...
	while(1)
	{
		...
		printf("Enter choice : ");
		//scanf("%d", &choice);
		if((Get_Validate_Input_Number(&choice, input_str, STR_MAX_CHARS, FSM_OPER, EXIT_OPER)) != SUCCESS)
		{
			choice = EXIT_OPER + 1;
		}
		switch(choice)
...
uint16_t FSM_StartUp_Proc(const uint8_t ctrl_elevator_ch_id)
{
	...
	//scanf("%u", &cur_floor);
	if((Get_Validate_Input_Number(&cur_floor, input_str, STR_MAX_CHARS, MIN_NUM_FLOORS , MAX_NUM_FLOORS)) != SUCCESS)
	{
...
{
	elevator_ctrl_and_status_t *cur_elevator_ptr;
	uint16_t ret_status, cur_floor;
	uint8_t limit_sw_cur_floor, limit_sw_min_floor, limit_sw_max_floor, car_movement_state;
...
{
    elevator_ctrl_and_status_t *cur_elevator_ptr;
    uint16_t ret_status, floor_type;
	uint32_t floor_call;
	...
	switch(cur_elevator_ptr->before_fsm_state)
	{
        ...
        case FSM_PREPARE_USER_ENTRY_AND_EXIT:
			...
			//scanf("%u", &floor_type);
			if((Get_Validate_Input_Number(&floor_type, input_str, STR_MAX_CHARS, IN_CAR_FLOOR_CALL_OPER, HALL_DOWN_FLOOR_CALL_OPER)) != SUCCESS)
	        {
				 appl_status_flag = ERR_FORMAT_INVALID ;
			...
			switch(floor_type)
			{
				...
				case HALL_DOWN_FLOOR_CALL_OPER:
	               ...
	               // scanf("%u", &floor_call);
				   if((Get_Validate_Input_Number(&floor_call, input_str, STR_MAX_CHARS, MIN_NUM_FLOORS, MAX_NUM_FLOORS)) != SUCCESS)
	               {
				       appl_status_flag = ERR_FORMAT_INVALID ;
		               Error_or_Warning_Proc("11.21.03", ERROR_OCCURED, appl_status_flag);
	                   return FAILURE;
					...
					if((ret_status = Validate_Floor(floor_call)) != SUCCESS)
					{
	                    ...
	                    Error_or_Warning_Proc("11.21.04", ERROR_OCCURED, appl_status_flag);
		                return appl_status_flag;
					}
					if(cur_elevator_ptr->elevator_status == STARTUP_STATIONARY || cur_elevator_ptr->elevator_status == MOVED_UP_STATIONARY || cur_elevator_ptr->elevator_status == MOVED_DOWN_STATIONARY)
					{
					    if(floor_call == cur_elevator_ptr->cur_floor)
	                    {
	                        ...
	                        Error_or_Warning_Proc("11.21.05", WARNING_OCCURED, appl_status_flag);
	                        return appl_status_flag;
	                   }
					}
				break;
				default:
				   ...
				   appl_status_flag = ERR_FORMAT_INVALID ;
	               Error_or_Warning_Proc("11.21.06", ERROR_OCCURED, appl_status_flag);
		           return appl_status_flag;
			}
			switch(floor_type)
			{
...
uint16_t Appl_Reset_Proc(const uint8_t cur_ctrl_elevator_ch_id)
{
    ...
    if((Get_Validate_Input_Number(&enter_num_floors, input_str, STR_MAX_CHARS, MIN_NUM_FLOORS, MAX_NUM_FLOORS)) != SUCCESS)
	{
	      appl_status_flag = ERR_FORMAT_INVALID ;
	      Error_or_Warning_Proc("11.21.03", ERROR_OCCURED, appl_status_flag);
...
{
	 int64_t num = 0, *num_conv_from_str;
	 uint64_t place;
	 int64_t cur_unit;
     ...
     if(num_conv_from_str == NULL_PTR || num_in_str == NULL_PTR )
	 {
	  ...
	  num_conv_from_str = (int64_t *) num_conv_from_str_ptr;
	   *num_conv_from_str = 0;
...
uint16_t Get_Validate_Input_Number(void *const int32_input_num_ptr, char *const input_str_ptr, const unsigned int input_str_max_chars, const int64_t valid_min_value, const int64_t valid_max_value)
{
	int64_t temp_int, *temp_int_ptr;

    if(int32_input_num_ptr == NULL_PTR || input_str_ptr == NULL_PTR || input_str_max_chars <= 1)
	{
		#ifdef TRACE_ERROR
		   printf("ERR: data are null ptr \n");
		#endif
    ...
    temp_int_ptr = (int64_t *)int32_input_num_ptr;
	*temp_int_ptr = 0;
	...
	{
		 #ifdef TRACE_ERROR
		      printf("ERR: input data - %ld, out of range [%ld,%ld] \n", temp_int, valid_min_value, valid_max_value);
		 #endif
		 return FAILURE;
	}
...
uint16_t Get_Input_Str(char *const input_str_ptr, const unsigned int input_str_max_chars)
{
	...
	if(input_str_ptr == NULL_PTR || input_str_max_chars <= 1)
	{
		 #ifdef TRACE_ERROR
		   printf("ERR: data are null ptr \n");
		#endif
